...
def normalize_cifar10_images(X):
    mean_rgb = np.array(
        [[[[0.4914 * 255, 0.4822 * 255, 0.4465 * 255]]]], dtype=np.float32
    )
    std_rgb = np.array(
        [[[[0.2470 * 255, 0.2435 * 255, 0.2616 * 255]]]], dtype=np.float32
    )
...
def normalize_cinic10_images(X):
    mean_rgb = np.array(
        [[[[0.47889522 * 255, 0.47227842 * 255, 0.43047404 * 255]]]], dtype=np.float32
    )
    std_rgb = np.array(
        [[[[0.24205776 * 255, 0.23828046 * 255, 0.25874835 * 255]]]], dtype=np.float32
    )
    X = (X.astype(np.float32) - mean_rgb) / std_rgb
    return X
...
def load_cifar10(args):
    ...
    num_classes = 10
    X_train, X_test = (
        normalize_cifar10_images(X_train),
        normalize_cifar10_images(X_test),
    )
...
def load_cifar100(args):
    ...
    num_classes = 100
    X_train, X_test = (
        normalize_cifar100_images(X_train),
        normalize_cifar100_images(X_test),
    )
    Y_train, Y_test = one_hot(Y_train, num_classes), one_hot(Y_test, num_classes)
    # sort by class
    X_train, Y_train = sort_by_class(X_train, Y_train)
...
def load_cinic10(args):
    ...
    num_classes = 10
    X_train, X_test = (
        normalize_cinic10_images(X_train),
        normalize_cinic10_images(X_test),
    )
    Y_train, Y_test = one_hot(Y_train, num_classes), one_hot(Y_test, num_classes)
    # sort by class
    X_train, Y_train = sort_by_class(X_train, Y_train)
...
def subset_train_idxs_randomly(I, args):
    ...
    I = np.sort(rng.choice(I.shape[0], args.subset_size, replace=False)).astype(
        np.int32
    )
...
def load_data(args):
    ...
    # Add support for using a fraction of the dataset
    if hasattr(args, "dataset_fraction") and args.dataset_fraction < 1.0:
        # Calculate sizes for both train and test sets
        num_train = int(X_train.shape[0] * args.dataset_fraction)
        num_test = int(X_test.shape[0] * args.dataset_fraction)

        # Ensure the numbers are divisible by number of classes
        num_classes = Y_train.shape[1]
        train_examples_per_class = num_train // num_classes
        test_examples_per_class = num_test // num_classes
        num_train = train_examples_per_class * num_classes
        num_test = test_examples_per_class * num_classes

        logger.info(
            f"Using {args.dataset_fraction:.1%} of the dataset "
            f"(train: {num_train} examples, test: {num_test} examples)"
        )

        # Apply balanced sampling to both sets
        X_train, Y_train = get_class_balanced_random_subset(
            X_train, Y_train, cls_smpl_sz=train_examples_per_class, seed=args.train_seed
        )
        X_test, Y_test = get_class_balanced_random_subset(
            X_test, Y_test, cls_smpl_sz=test_examples_per_class, seed=args.train_seed
        )

        # Update args with new dataset sizes
        args = update_data_args(args, X_train, Y_train, X_test, Y_test)

    if not hasattr(args, "random_label_fraction"):
        args.random_label_fraction = 0
    if args.random_label_fraction > 0:
        X_train, Y_train = randomize_labels(
            X_train, Y_train, args.random_label_fraction, args.random_label_seed
        )
