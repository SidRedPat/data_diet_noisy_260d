...
def record_prune_stats(rec, prune_stats):
    rec.prune_steps = prune_stats["steps"]
    rec.pruned_percentages = prune_stats["pruned_percentages"]
    rec.pruned_sizes = prune_stats["pruned_sizes"]
    rec.total_sizes = prune_stats["total_sizes"]
    return rec
...
def save_recorder(save_dir, rec, verbose=True):
    ...
    with open(save_path, "wb") as f:
        pickle.dump(vars(rec), f)
    if verbose:
        print(f"Save record to {save_path}")
    return save_path
...
def load_recorder(load_dir, verbose=True):
    ...
    with open(load_path, "rb") as f:
        rec = SimpleNamespace(**pickle.load(f))
    if verbose:
        print(f"Load record from {load_path}")
    return rec
